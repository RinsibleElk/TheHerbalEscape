//
//  Colors.swift
//  TheHerbalEscape
//
//  Created by Oliver Samson on 25/11/2017.
//  Copyright © 2017 Oliver Samson. All rights reserved.
//

import UIKit

/// Colors used in this application.
struct Colors {
    static let SlightlyDarkForestGreen = UIColor(rgb:0x158015)
    static let ForestGreen = UIColor(rgb:0x228b22)
    static let DarkForestGreen = UIColor(rgb:0x0d4b02)
    static let DarkRed = UIColor(rgb:0x800000)
    static let Black = UIColor.black
    static let White = UIColor.white
    static let Blue = UIColor.blue
    static let Green = UIColor.green
    static let Purple = UIColor.purple
    static let Yellow = UIColor.yellow
    static let Pink = UIColor.magenta
    static let Brown = UIColor.brown
    static let Orange = UIColor.orange
    static let Azure = UIColor(rgb: 0xb3ffff)
    static let Lavender = UIColor(rgb: 0xe6e6fa)
    static let MistyRose = UIColor(rgb: 0xffe4e1)
    static let LemonChiffon = UIColor(rgb: 0xfffacd)
    static let Honeydew = UIColor(rgb: 0xb3ffb3)
    
    // Quiz answers
    static let QuizGreenBorder = UIColor(rgb: 0x228b22)
    static let QuizGreenBackground = UIColor(rgb: 0xb4eeb4)
    static let QuizRedBorder = UIColor(rgb: 0x8b2222)
    static let QuizRedBackground = UIColor(rgb: 0xeeb4b4)
    static let QuizOrangeBorder = UIColor.orange
}
