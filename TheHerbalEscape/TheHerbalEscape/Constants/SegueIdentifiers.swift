//
//  SegueIdentifiers.swift
//  TheHerbalEscape
//
//  Created by Oliver Samson on 25/11/2017.
//  Copyright © 2017 Oliver Samson. All rights reserved.
//

import Foundation

/// Segue identifiers that need to be referenced in code.
struct SegueIdentifiers {
    /// To transition at the end of the flashcards experience.
    static let Flashcard1Identifier = "Flashcard1Identifier"
    static let Flashcard2Identifier = "Flashcard2Identifier"
    static let BrowserTableEmbedSegueIdentifier = "BrowserTableEmbedSegueIdentifier"
    static let BrowserContentEmbedSegueIdentifier = "BrowserContentEmbedSegueIdentifier"
    static let FlashcardsSegueIdentifier = "FlashcardsSegueIdentifier"
    static let QuizSegueIdentifier = "QuizSegueIdentifier"
    static let FlashcardsMenuSegueIdentifier = "FlashcardsMenuSegueIdentifier"
    static let QuizMenuSegueIdentifier = "QuizMenuSegueIdentifier"
    static let QuizQuestion1SegueIdentifier = "QuizQuestion1SegueIdentifier"
    static let QuizQuestion2SegueIdentifier = "QuizQuestion2SegueIdentifier"
}
